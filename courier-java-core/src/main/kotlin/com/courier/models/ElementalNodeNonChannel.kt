...
private constructor(
    ...
    private constructor(
        ...
        class Builder internal constructor() {
             ...
             * `tertiary` are the other two button styles, and `link` renders as inline text rather
             * than a button.
             *
             ...
             * outlines `secondary`, and draws `tertiary` as a solid button. Slack renders all three
             * as Block Kit buttons, with `secondary` in Slack's `primary` style and `tertiary` in
             * its `danger` style.
             *
             * `background_color` is the fill for `button`, and the border and label color for
             * `secondary`. For `tertiary` it colors the underline and label in email and the fill
             * in the Inbox. It does not apply to `link`. An Inbox theme that sets its own action
             * colors takes precedence over the template.
             */
