...
sealed class JsonField<out T : Any> {
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw CourierInvalidDataException("`$name` is not set")
            is JsonNull -> throw CourierInvalidDataException("`$name` is null")
            else -> throw CourierInvalidDataException("`$name` is invalid, received $this")
        }
